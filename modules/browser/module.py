...
def _escape_for_python(s: str) -> str:
    """安全转义字符串用于Python脚本中的字符串字面量"""
    if s is None:
        return ""
    # 使用json.dumps进行完整转义，然后去掉外层引号
    return json.dumps(s, ensure_ascii=False)[1:-1]


def _escape_url(url: str) -> str:
    """安全转义URL"""
    return _escape_for_python(url)


def _escape_selector(selector: str) -> str:
    """安全转义CSS选择器"""
    return _escape_for_python(selector)


def _escape_value(value: str) -> str:
    """安全转义输入值"""
    return _escape_for_python(value)


def _encode_script(code: str) -> str:
    """使用base64编码脚本代码，避免注入问题"""
...
def _run_browser_script(script: str, timeout: int = 60000) -> str:
    ...
    # 使用UUID生成唯一文件名，避免竞争条件
    script_file = BROWSER_DIR / f"temp_script_{uuid.uuid4().hex}.py"
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto('{safe_url}', wait_until='networkidle', timeout=60000)
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            page.goto('{safe_url}', wait_until='{wait_until}', timeout=60000)
            context.storage_state(path=str(r'{STATE_FILE}'))
...
def browser_snapshot(selector: str = None) -> str:
    """获取页面快照，返回可交互元素列表"""
    user_data_dir = STATE_DIR / "user_data"
    safe_selector = _escape_selector(selector) if selector else None
    selector_code = f"page.locator('{safe_selector}')" if safe_selector else "page"
...
def browser_click(selector: str, click_count: int = 1, delay: int = 0) -> str:
    """点击页面元素"""
    user_data_dir = STATE_DIR / "user_data"
    safe_selector = _escape_selector(selector)
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                element = page.locator('{safe_selector}')
                if {click_count} == 2:
...
def browser_fill(selector: str, value: str, clear: bool = True) -> str:
    ...
    user_data_dir = STATE_DIR / "user_data"
    safe_selector = _escape_selector(selector)
    safe_value = _escape_value(value)
...
def browser_screenshot(path: str = None, full_page: bool = False, 
    ...
    safe_selector = _escape_selector(selector) if selector else None
    safe_path = _escape_value(save_path)
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                if '{safe_selector}':
                    page.locator('{safe_selector}').screenshot(path='{safe_path}')
                else:
                    page.screenshot(path='{safe_path}', full_page={str(full_page).lower()})
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                text = page.locator('{safe_selector}').inner_text() if '{safe_selector}' else page.content()
...
def browser_evaluate(script_code: str) -> str:
    """在页面中执行 JavaScript - 使用base64编码避免注入"""
    user_data_dir = STATE_DIR / "user_data"
    
    # 使用base64编码JavaScript代码，在脚本中解码执行
    encoded_script = _encode_script(script_code)
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                # 解码并执行JavaScript
                js_code = base64.b64decode('{encoded_script}').decode('utf-8')
...
def main():
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                page.wait_for_selector('{safe_selector}', timeout={timeout}, state='{state}')
...
'''
        ...
        safe_name = _escape_value(name)
        safe_value = _escape_value(value)
        safe_domain = _escape_value(domain or "example.com")
...
def browser_form_fill(fields: list, submit: str = None) -> str:
    ...
    field_actions = []
    for f in fields:
        safe_sel = _escape_selector(f['selector'])
        safe_val = _escape_value(f['value'])
        field_actions.append(f"page.fill('{safe_sel}', '{safe_val}')")
    
    field_actions_code = "\n        ".join(field_actions)
    safe_submit = _escape_selector(submit) if submit else None
    submit_code = f"page.click('{safe_submit}')" if safe_submit else ""
...
def browser_extract(url: str, selectors: dict) -> str:
    ...
    user_data_dir = STATE_DIR / "user_data"
    safe_url = _escape_url(url)
    safe_selectors = json.dumps(selectors, ensure_ascii=False)
...
MODULE_DEF = {
    "name": "browser",
    "version": "2.1.0",
    "description": "浏览器自动化 v2.1 - 支持持久化会话，保持登录状态，安全转义处理",
    "tools": [
        {
            "name": "browser_open",
        ...
        },
        {
            ...
            "description": "在页面中执行 JavaScript (安全编码传输)",
            "input_schema": {
                "type": "object",
                "properties": {
