...
// 第十三阶段新�?const processRespawn = require('./process-respawn');
...
// 第十四阶段新�?const configEval = require('./config-eval');
...
// 第十五阶段新�?const normalizeSecretInput = require('./normalize-secret-input');
...
// 第十六阶段新�?const nodeMatch = require('./node-match');
...
// 第十七阶段新�?const secureRandom = require('./secure-random');
const spawnUtils = require('./spawn-utils');
// 第十八阶段新�?- 情感分析
const sentiment = require('./modules/sentiment');
const sectorRotation = require('./modules/sector-rotation');
const dataAggregator = require('./modules/data-aggregator');
const indicators = require('./modules/indicators');
// 第二十阶段新�?- 安全增强模块
const securityEnhanced = require('./modules/security-enhanced');

// 第二十一阶段新增 - Context Engine 插件化架�?const contextEngineRegistry = require('./modules/context-engine-registry');
const contextEngineLegacy = require('./modules/context-engine-legacy');

// 第二十二阶段新增 - Subagent 生命周期管理增强
const subagentLifecycle = require('./modules/subagent-lifecycle');

// 第二十三阶段新增 - Session Runtime Model Override
const sessionRuntimeModel = require('./modules/session-runtime-model');

// 第二十四阶段新增 - Skills Enhanced Module
const skillsEnhanced = require('./modules/skills-enhanced');

// 第二十五阶段新增 - Memory Vector Enhanced Module
const memoryVectorEnhanced = require('./modules/memory-vector-enhanced');

// 第二十六阶段新增 - 学习自 core-modules
const hooksAdvanced = require('./modules/hooks-advanced');
const toolPolicy = require('./modules/tool-policy');
const remoteSync = require('./modules/remote-sync');
...
function getModules() {
  return {
    ...
    // 第十八阶段新�?- 情感分析
    sentiment: Object.keys(sentiment).filter(k => typeof sentiment[k] === 'function'),
    sectorRotation: Object.keys(sectorRotation).filter(k => typeof sectorRotation[k] === 'function'),
    dataAggregator: Object.keys(dataAggregator).filter(k => typeof dataAggregator[k] === 'function'),
    indicators: Object.keys(indicators).filter(k => typeof indicators[k] === 'function'),
    // 第二十阶段新�?- 安全增强模块
    securityEnhanced: Object.keys(securityEnhanced).filter(k => typeof securityEnhanced[k] === 'function'),
    contextEngineLegacy: Object.keys(contextEngineLegacy).filter(k => typeof contextEngineLegacy[k] === 'function'),
    // 第二十二阶段新增 - Subagent 生命周期管理增强
    subagentLifecycle: Object.keys(subagentLifecycle).filter(k => typeof subagentLifecycle[k] === 'function'),
    // 第二十三阶段新增 - Session Runtime Model Override
    sessionRuntimeModel: Object.keys(sessionRuntimeModel).filter(k => typeof sessionRuntimeModel[k] === 'function'),
    // 第二十四阶段新增 - Skills Enhanced Module
    skillsEnhanced: Object.keys(skillsEnhanced).filter(k => typeof skillsEnhanced[k] === 'function'),
    // 第二十五阶段新增 - Memory Vector Enhanced Module
    memoryVectorEnhanced: Object.keys(memoryVectorEnhanced).filter(k => typeof memoryVectorEnhanced[k] === 'function'),
    // 第二十六阶段新增 - 学习自 core-modules
    hooksAdvanced: Object.keys(hooksAdvanced).filter(k => typeof hooksAdvanced[k] === 'function'),
    toolPolicy: Object.keys(toolPolicy).filter(k => typeof toolPolicy[k] === 'function'),
    remoteSync: Object.keys(remoteSync).filter(k => typeof remoteSync[k] === 'function')
  };
}
...
function getCapabilities() {
  return {
    ...
    // 第十五阶段新�?    normalizeSecretInput: ['normalizeSecretInput', 'normalizeOptionalSecretInput'],
    ...
    // 第十六阶段新�?    nodeMatch: ['normalizeNodeKey', 'resolveNodeMatches', 'resolveNodeIdFromCandidates'],
    ...
    // 第十八阶段新�?- 情感分析
    sentiment: ['analyzeSentiment', 'batchAnalyzeSentiment', 'tagMemoryWithSentiment', 'filterMemoriesBySentiment', 'getSentimentStats', 'getSentimentTrend'],
    // 补充缺失模块描述
...
machineName: ['getMachineDisplayName', 'getMachineDisplayNameSync', 'getHostname', 'getShortHostname', 'getMachineType', 'getMachineId', 'isLocalhost', 'getMachineInfo', 'clearMachineNameCache'],
    // 第十九阶段新�?- A股分析模�?    sectorRotation: ['analyzeSectorRotationByCapital', 'getHotStocks', 'analyzeSectorByIndustry', 'generateSectorRotationReport', 'predictNextHotSector', 'getCapitalFlowRanking', 'getDateRange'],
    dataAggregator: ['getStockFromPostgres', 'getPricesFromPostgres', 'getFlowsFromPostgres', 'getStockFromAkshare', 'getPricesFromAkshare', 'getRealtimeFromEastmoney', 'getStockInfo', 'getAggregatedPrices', 'getAggregatedFlows', 'getComprehensiveStockData', 'healthCheck', 'getDataSourcesStatus'],
    indicators: ['calculateMA', 'calculateEMA', 'calculateMACD', 'calculateKDJ', 'calculateRSI', 'calculateBollingerBands', 'calculateVolumeAnalysis', 'calculateOBV', 'analyzeOBVTrend', 'calculateATR', 'detectMACDDivergence', 'detectKDJSignal', 'detectBollBreakthrough', 'detectCandlePatterns', 'calculateVolumeSaturation', 'calculateLargeOrderNet', 'calculateCapitalFlowRatio', 'analyzeMainCapital', 'generateIndicatorReport', 'analyzeTrend', 'detectOverboughtOversold', 'generateSignals', 'fetchCapitalFlowRanking', 'fetchStockCapitalFlow', 'fetchMultipleStocksCapitalFlow', 'analyzeCapitalFlowData', 'generateCapitalFlowReport', 'formatMoney', 'filterCapitalInflow', 'filterCapitalOutflow', 'fetchSectorCapitalFlow'],
    // 第二十一阶段新增 - Context Engine 插件化架�?    contextEngineRegistry: ['ContextEngineInfo', 'ContextEngineFactory', 'registerContextEngine', 'getContextEngineFactory', 'listContextEngineIds', 'resolveContextEngine', 'setDefaultContextEngine', 'getDefaultContextEngineId'],
    contextEngineLegacy: ['LegacyContextEngine', 'AssembleResult', 'CompactResult', 'IngestResult', 'BootstrapResult'],
    // 第二十二阶段新增 - Subagent 生命周期管理增强
    subagentLifecycle: ['LifecycleEventType', 'EndReason', 'SubagentStatus', 'LifecycleEvent', 'CompletionMessageFlow', 'SubagentLifecycleManager', 'getLifecycleManager', 'setLifecycleManager'],
    // 第二十三阶段新增 - Session Runtime Model Override
    sessionRuntimeModel: ['ModelConfig', 'SessionModelOverride', 'SessionRuntimeModelManager', 'getSessionRuntimeModelManager', 'setSessionRuntimeModelManager'],
    // 第二十四阶段新增 - Skills Enhanced Module
    skillsEnhanced: ['SkillVersion', 'SkillCacheEntry', 'SkillCacheManager', 'SkillDependencyNode', 'SkillDependencyGraph', 'SkillEnvironment', 'SkillEnvironmentManager', 'ensureDirectories'],
    // 第二十五阶段新增 - Memory Vector Enhanced Module
    memoryVectorEnhanced: ['EmbeddingProviderType', 'EmbeddingProviderConfig', 'EmbeddingProvider', 'VectorDocument', 'VectorIndex', 'TimeDecay', 'VectorCompressor', 'VectorStore', 'ensureDirectories'],
    // 第二十六阶段新增 - 学习自 core-modules
    hooksAdvanced: ['HooksAdvancedManager', 'HookConditionEvaluator', 'HookExecutor', 'HOOK_TYPES', 'DEFAULT_MEMORY_FLUSH_CONFIG', 'createManager'],
    toolPolicy: ['ToolPolicyPipeline', 'ToolSchemaNormalizer', 'ToolRegistry', 'TOOL_PROFILES', 'TOOL_CATEGORIES', 'createPipeline', 'createRegistry', 'createNormalizer'],
    remoteSync: ['RemoteSyncManager', 'MemorySync', 'ConversationSync', 'RemoteAPIClient', 'createManager']
  };
}
...
module.exports = {
  ...
  // 第十八阶段新�?- 情感分析
  sentiment,
  // 第十九阶段新�?- A股分析模�?  sectorRotation,
  dataAggregator,
  indicators,
  // 第二十阶段新�?- 安全增强模块
  securityEnhanced,
  // 第二十一阶段新增 - Context Engine 插件化架�?  contextEngineRegistry,
  contextEngineLegacy,
  // 第二十二阶段新增 - Subagent 生命周期管理增强
  subagentLifecycle,
  // 第二十三阶段新增 - Session Runtime Model Override
