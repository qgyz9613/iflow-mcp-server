/**
 * 多数据源聚合模块
 * 整合PostgreSQL数据库、AKShare、东方财富API等多个数据源
 * 
 * 更新日志:
 * - 2026-03-20: 添加PostgreSQL连接池支持，密码从环境变量/secrets获取
 */

const fs = require('fs');
const path = require('path');
const { Pool } = require('pg');

// ==================== PostgreSQL 连接池配置 ====================

/**
 * 获取PostgreSQL密码
 * 优先级: 环境变量 > secrets模块 > 默认值
 */
async function getPgPassword() {
  // 1. 检查环境变量
  if (process.env.PG_PASSWORD) {
    return process.env.PG_PASSWORD;
  }
  
  // 2. 尝试从secrets模块获取
  try {
    const secrets = require('./secrets.js');
    const secret = await secrets.getSecret('pg_password');
    if (secret) {
      return secret;
    }
  } catch (err) {
    // secrets模块不可用，继续使用默认值
  }
  
  // 3. 尝试从secrets.json读取
  try {
    const secretsPath = path.join(process.env.USERPROFILE || process.env.HOME, '.iflow', 'secrets.json');
    const content = fs.readFileSync(secretsPath, 'utf8');
    const secretsData = JSON.parse(content);
    if (secretsData.pg_password) {
      return secretsData.pg_password;
    }
  } catch (err) {
    // secrets.json不可用，使用默认值
  }
  
  // 4. 返回默认密码（硬编码备用）
  return 'SX7YRCwXszkdtd4B';
}

// 连接池实例（单例模式）
let poolInstance = null;
let poolConfig = null;

/**
 * 初始化PostgreSQL连接池
 * @param {Object} options - 可选配置覆盖
 * @returns {Promise<Pool>} 连接池实例
 */
async function initPool(options = {}) {
  if (poolInstance) {
    return poolInstance;
  }
  
  const password = await getPgPassword();
  
  poolConfig = {
    host: options.host || process.env.PG_HOST || '38.165.21.21',
    port: options.port || parseInt(process.env.PG_PORT) || 5432,
    database: options.database || process.env.PG_DATABASE || 'postgres',
    user: options.user || process.env.PG_USER || 'admin',
    password: password,
    
    // 连接池配置
    max: options.max || 20,                    // 最大连接数
    min: options.min || 2,                     // 最小连接数
    idleTimeoutMillis: options.idleTimeoutMillis || 30000,  // 空闲连接超时30秒
    connectionTimeoutMillis: options.connectionTimeoutMillis || 5000,  // 连接超时5秒
    
    // 连接验证
    allowExitOnIdle: false,  // 空闲时不退出进程
    
    // SSL配置（远程服务器可能需要）
    ssl: options.ssl || { rejectUnauthorized: false }
  };
  
  poolInstance = new Pool(poolConfig);
  
  // 错误处理
  poolInstance.on('error', (err, client) => {
    console.error('[PostgreSQL Pool] Unexpected error on idle client:', err.message);
  });
  
  // 连接事件
  poolInstance.on('connect', (client) => {
    console.log('[PostgreSQL Pool] New client connected');
  });
  
  // 移除事件
  poolInstance.on('remove', (client) => {
    console.log('[PostgreSQL Pool] Client removed from pool');
  });
  
  return poolInstance;
}

/**
 * 获取连接池实例
 * @returns {Pool} 连接池实例
 */
function getPool() {
  if (!poolInstance) {
    throw new Error('Pool not initialized. Call initPool() first.');
  }
  return poolInstance;
}

/**
 * 关闭连接池
 */
async function closePool() {
  if (poolInstance) {
    await poolInstance.end();
    poolInstance = null;
    poolConfig = null;
    console.log('[PostgreSQL Pool] Pool closed');
  }
}

/**
 * 获取连接池状态
 * @returns {Object} 连接池状态信息
 */
function getPoolStatus() {
  if (!poolInstance) {
    return {
      initialized: false,
      message: 'Pool not initialized'
    };
  }
  
  return {
    initialized: true,
    totalCount: poolInstance.totalCount,       // 总连接数
    idleCount: poolInstance.idleCount,         // 空闲连接数
    waitingCount: poolInstance.waitingCount,   // 等待获取连接的请求数
    config: {
      host: poolConfig?.host,
      port: poolConfig?.port,
      database: poolConfig?.database,
      user: poolConfig?.user,
      max: poolConfig?.max,
      min: poolConfig?.min
    },
    health: {
      status: poolInstance.idleCount >= 0 ? 'ok' : 'error',
      availableConnections: poolInstance.idleCount,
      usedConnections: poolInstance.totalCount - poolInstance.idleCount,
      pendingRequests: poolInstance.waitingCount
    }
  };
}

/**
 * 执行SQL查询（使用连接池）
 * @param {string} query - SQL查询
 * @param {Array} params - 参数
 * @returns {Promise<Object>} 查询结果
 */
async function executeQuery(query, params = []) {
  const pool = getPool();
  const start = Date.now();
  
  try {
    const result = await pool.query(query, params);
    const duration = Date.now() - start;
    
    return {
      success: true,
      rows: result.rows,
      rowCount: result.rowCount,
      duration: duration,
      fields: result.fields?.map(f => f.name)
    };
  } catch (error) {
    const duration = Date.now() - start;
    console.error(`[PostgreSQL] Query failed in ${duration}ms:`, error.message);
    
    return {
      success: false,
      error: error.message,
      code: error.code,
      duration: duration
    };
  }
}

/**
 * 执行事务
 * @param {Function} callback - 事务回调函数
 * @returns {Promise<any>} 事务结果
 */
async function executeTransaction(callback) {
  const pool = getPool();
  const client = await pool.connect();
  
  try {
    await client.query('BEGIN');
    const result = await callback(client);
    await client.query('COMMIT');
    return { success: true, result };
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('[PostgreSQL] Transaction failed:', error.message);
    return { success: false, error: error.message };
  } finally {
    client.release();
  }
}

// 兼容旧代码的配置对象（已废弃，保留向后兼容）
const PG_CONFIG = {
  host: '38.165.21.21',
  port: 5432,
  database: 'postgres',
  user: 'admin',
  password: 'SX7YRCwXszkdtd4B'
};

/**
 * 数据源配置
 */
const DATA_SOURCES = {
  postgres: {
    name: 'PostgreSQL数据库',
    enabled: true,
    priority: 1,
    features: ['stock_info', 'flows', 'labels', 'prices']
  },
  akshare: {
    name: 'AKShare',
    enabled: true,
    priority: 2,
    features: ['realtime', 'history', 'financial']
  },
  eastmoney: {
    name: '东方财富',
    enabled: true,
    priority: 3,
    features: ['realtime', 'fund_flow']
  }
};

/**
 * 从PostgreSQL获取股票信息（使用连接池）
 */
async function getStockFromPostgres(tsCode) {
  const query = `
    SELECT 
      s.ts_code,
      s.name,
      s.industry,
      s.exchange,
      s.market_type,
      s.list_date
    FROM stocks s
    WHERE s.ts_code = $1;
  `;
  
  const result = await executeQuery(query, [tsCode]);
  
  if (!result.success) {
    return {
      source: 'postgres',
      available: false,
      error: result.error
    };
  }
  
  if (result.rows.length > 0) {
    return {
      source: 'postgres',
      data: result.rows[0],
      available: true,
      duration: result.duration
    };
  } else {
    return {
      source: 'postgres',
      available: false,
      message: '股票不存在'
    };
  }
}

/**
 * 从PostgreSQL获取价格数据（使用连接池）
 */
async function getPricesFromPostgres(tsCode, startDate, endDate) {
  const query = `
    SELECT 
      ts_code,
      trade_date,
      open,
      high,
      low,
      close,
      pre_close,
      pct_chg,
      vol,
      amount
    FROM stock_prices
    WHERE ts_code = $1
      AND trade_date BETWEEN $2::date AND $3::date
    ORDER BY trade_date DESC;
  `;
  
  const result = await executeQuery(query, [tsCode, startDate, endDate]);
  
  if (!result.success) {
    return {
      source: 'postgres',
      error: result.error,
      data: []
    };
  }
  
  return {
    source: 'postgres',
    data: result.rows,
    count: result.rows.length,
    duration: result.duration
  };
}

/**
 * 从PostgreSQL获取资金流向数据（使用连接池）
 */
async function getFlowsFromPostgres(tsCode, startDate, endDate) {
  const query = `
    SELECT 
      ts_code,
      trade_date,
      main_net_inflow,
      main_net_ratio,
      super_net,
      super_ratio,
      big_net,
      mid_net,
      small_net
    FROM stock_flows
    WHERE ts_code = $1
      AND trade_date BETWEEN $2::date AND $3::date
    ORDER BY trade_date DESC;
  `;
  
  const result = await executeQuery(query, [tsCode, startDate, endDate]);
  
  if (!result.success) {
    return {
      source: 'postgres',
      error: result.error,
      data: []
    };
  }
  
  return {
    source: 'postgres',
    data: result.rows,
    count: result.rows.length,
    duration: result.duration
  };
}

/**
 * 使用AKShare获取股票数据
 */
async function getStockFromAkshare(tsCode) {
  try {
    // 检查AKShare是否安装
    const { execSync } = require('child_process');
    execSync('pip show akshare > nul 2>&1');

    const { exec } = require('child_process');
    const script = `
import akshare as ak
result = ak.stock_info(ts_code='${ts_code}')
print(result.to_json())
    `;

    const result = execSync(`python -c "${script}"`, { encoding: 'utf8' });
    const data = JSON.parse(result.stdout);

    return {
      source: 'akshare',
      data: data,
      available: true
    };
  } catch (error) {
    return {
      source: 'akshare',
      available: false,
      error: error.message
    };
  }
}

/**
 * 使用AKShare获取价格数据
 */
async function getPricesFromAkshare(tsCode, startDate, endDate) {
  try {
    const { exec } = require('child_process');
    const script = `
import akshare as ak
import pandas as pd
result = ak.stock_zh_a_hist(symbol='${tsCode}', period='daily', start_date='${startDate}', end_date='${endDate}')
print(result.to_json())
    `;
    const result = execSync(`python -c "${script}"`, { encoding: 'utf8' });
    const data = JSON.parse(result.stdout);

    return {
      source: 'akshare',
      data: data,
      count: data.length || 0
    };
  } catch (error) {
    return {
      source: 'akshare',
      error: error.message,
      data: []
    };
  }
}

/**
 * 使用东方财富获取实时数据
 */
async function getRealtimeFromEastmarket(tsCode) {
  try {
    const { exec } = require('child_process');
    const script = `
import akshare as ak
result = ak.stock_zh_a_spot_em()
stock = result[result['代码'] == '${tsCode}']
print(stock.to_json())
    `;
    const result = execSync(`python -c "${script}"`, { encoding: 'utf8' });
    const data = JSON.parse(result.stdout);

    return {
      source: 'eastmoney',
      data: data,
      available: true
    };
  } catch (error) {
    return {
      source: 'eastmoney',
      available: false,
      error: error.message
    };
  }
}

/**
 * 聚合股票信息（优先使用数据库）
 */
async function getStockInfo(tsCode) {
  // 按优先级尝试各个数据源
  const sources = [
    { name: 'postgres', fn: () => getStockFromPostgres(tsCode) },
    { name: 'akshare', fn: () => getStockFromAkshare(tsCode) },
    { name: 'eastmoney', fn: () => getRealtimeFromEastmarket(tsCode) }
  ];

  for (const source of sources) {
    if (!DATA_SOURCES[source.name].enabled) continue;

    try {
      const result = await source.fn();
      if (result.available) {
        return {
          ...result,
          primary_source: source.name
        };
      }
    } catch (error) {
      console.error(`${source.name} error:`, error.message);
    }
  }

  return {
    available: false,
    message: '所有数据源均无法获取数据'
  };
}

/**
 * 聚合价格数据（优先使用数据库，然后AKShare）
 */
async function getAggregatedPrices(tsCode, startDate, endDate) {
  const sources = [
    { name: 'postgres', fn: () => getPricesFromPostgres(tsCode, startDate, endDate) },
    { name: 'akshare', fn: () => getPricesFromAkshare(tsCode, startDate, endDate) }
  ];

  for (const source of sources) {
    if (!DATA_SOURCES[source.name].enabled) continue;

    try {
      const result = await source.fn();
      if (result.data && result.data.length > 0) {
        return {
          ...result,
          primary_source: source.name,
          merged_from: [source.name]
        };
      }
    } catch (error) {
      console.error(`${source.name} error:`, error.message);
    }
  }

  return {
    data: [],
    primary_source: 'none',
    merged_from: []
  };
}

/**
 * 聚合资金流向数据
 */
async function getAggregatedFlows(tsCode, startDate, endDate) {
  const sources = [
    { name: 'postgres', fn: () => getFlowsFromPostgres(tsCode, startDate, endDate) }
  ];

  for (const source of sources) {
    if (!DATA_SOURCES[source.name].enabled) continue;

    try {
      const result = await source.fn();
      if (result.data && result.data.length > 0) {
        return {
          ...result,
          primary_source: source.name
        };
      }
    } catch (error) {
      console.error(`${source.name} error:`, error.message);
    }
  }

  return {
    data: [],
    primary_source: 'none'
  };
}

/**
 * 获取数据源状态
 */
function getDataSourcesStatus() {
  return {
    sources: DATA_SOURCES,
    total: Object.keys(DATA_SOURCES).length,
    enabled: Object.values(DATA_SOURCES).filter(s => s.enabled).length,
    disabled: Object.values(DATA_SOURCES).filter(s => !s.enabled).length
  };
}

/**
 * 更新数据源配置
 */
function updateDataSourceConfig(sourceName, config) {
  if (DATA_SOURCES[sourceName]) {
    DATA_SOURCES[sourceName] = {
      ...DATA_SOURCES[sourceName],
      ...config
    };
    return {
      success: true,
      updated: sourceName,
      config: DATA_SOURCES[sourceName]
    };
  } else {
    return {
      success: false,
      error: `数据源 ${sourceName} 不存在`
    };
  }
}

/**
 * 批量获取股票信息
 */
async function batchGetStockInfo(tsCodes) {
  const results = [];

  for (const tsCode of tsCodes) {
    const result = await getStockInfo(tsCode);
    results.push({
      ts_code: tsCode,
      ...result
    });
  }

  return {
    total: tsCodes.length,
    success: results.filter(r => r.available).length,
    results: results
  };
}

/**
 * 数据源健康检查（使用连接池）
 */
async function healthCheck() {
  const healthStatus = {};

  // 检查PostgreSQL连接池
  try {
    // 确保连接池已初始化
    if (!poolInstance) {
      await initPool();
    }
    
    const result = await executeQuery('SELECT 1 as test');
    
    if (result.success) {
      const poolStatus = getPoolStatus();
      healthStatus.postgres = { 
        status: 'ok', 
        message: '连接池正常',
        pool: {
          total: poolStatus.totalCount,
          idle: poolStatus.idleCount,
          waiting: poolStatus.waitingCount
        }
      };
    } else {
      healthStatus.postgres = { 
        status: 'error', 
        message: result.error 
      };
    }
  } catch (error) {
    healthStatus.postgres = { status: 'error', message: error.message };
  }

  // 检查AKShare
  try {
    const { execSync } = require('child_process');
    execSync('python -c "import akshare as ak"', { encoding: 'utf8' });
    healthStatus.akshare = { status: 'ok', message: 'AKShare可用' };
  } catch (error) {
    healthStatus.akshare = { status: 'error', message: 'AKShare不可用' };
  }

  // 检查东方财富
  try {
    const { execSync } = require('child_process');
    execSync('python -c "import akshare as ak; ak.stock_zh_a_spot_em()"', { 
      encoding: 'utf8',
      timeout: 10000 
    });
    healthStatus.eastmoney = { status: 'ok', message: '东方财富API可用' };
  } catch (error) {
    healthStatus.eastmoney = { status: 'error', message: '东方财富API不可用' };
  }

  return {
    timestamp: new Date().toISOString(),
    sources: healthStatus,
    overall: Object.values(healthStatus).filter(s => s.status === 'ok').length
  };
}

module.exports = {
  // PostgreSQL连接池管理
  initPool,
  getPool,
  closePool,
  getPoolStatus,
  executeQuery,
  executeTransaction,
  getPgPassword,
  
  // 数据源查询
  getStockFromPostgres,
  getPricesFromPostgres,
  getFlowsFromPostgres,
  getStockFromAkshare,
  getPricesFromAkshare,
  getRealtimeFromEastmarket,
  
  // 聚合查询
  getStockInfo,
  getAggregatedPrices,
  getAggregatedFlows,
  batchGetStockInfo,
  
  // 配置管理
  getDataSourcesStatus,
  updateDataSourceConfig,
  healthCheck,
  
  // 向后兼容
  PG_CONFIG
};