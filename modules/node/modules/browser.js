...
// 批量点击操作
async function batchClick(actions, options = {}) {
  const start = Date.now();
  try {
    if (!page) throw new Error('Browser not initialized');
    
    const results = [];
    const defaultDelay = options.delay || 100;
    ...
    for (const action of actions) {
      try {
        ...
        if (action.clickCount === 2) {
          await element.dblclick({ delay: action.delay || defaultDelay });
        } else {
          await element.click({ 
...
// 批量等待元素
async function batchWait(conditions, options = {}) {
  const start = Date.now();
  try {
    if (!page) throw new Error('Browser not initialized');
    ...
    for (const condition of conditions) {
      try {
        ...
        const timeout = condition.timeout || defaultTimeout;
...
// 批量获取文本
async function batchGetText(selectors, options = {}) {
  const start = Date.now();
  try {
    if (!page) throw new Error('Browser not initialized');
...
// 添加延迟任务
function addToDelayQueue(action, delay = 0) {
  delayQueue.add(action, delay);
  return { status: 'ok', queueSize: delayQueue.queue.length };
}

// 执行延迟队列
async function executeDelayQueue() {
  return await delayQueue.execute();
}

// 获取队列状态
function getDelayQueueStatus() {
  return {
    status: 'ok',
    queueSize: delayQueue.queue.length,
    isProcessing: delayQueue.isProcessing
  };
}
