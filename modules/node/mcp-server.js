...
const moduleDescriptions = {
  ...
  stableNodePath: '稳定Node路径（处理Homebrew版本升级、Homebrew Cellar路径解析）',
  machineName: '机器名称（获取机器显示名称、主机名、机器类型、唯一标识）',
  sentiment: '情感分析（看涨/看跌/中性、情感统计、情感趋势、情感搜索）',
  ...
  // 第十九阶段新增 - A股分析模块
  sectorRotation: '板块轮动分析（基于PostgreSQL数据库的资金流向、热点股票、行业板块分析、轮动报告、热点预测）',
  dataAggregator: '多数据源聚合（PostgreSQL数据库/AKShare/东方财富、优先级回退机制、健康检查、综合数据获取）',
      indicators: '技术指标增强（MA/EMA/MACD/KDJ/RSI/BOLL/ATR/OBV、MACD背离检测、KDJ超买超卖、OBV资金流向、BOLL突破、K线形态识别、量能饱和度、大单净量、资金流量比、主力资金分析、趋势分析、超买超卖检测、信号生成、东方财富API资金流向、主力净流入排名、个股历史资金流向、资金筛选）',
  // 第二十阶段新增 - 安全增强模块
  securityEnhanced: '安全增强机制（零宽度字符规范化、PowerShell命令安全检测、包管理器执行检测、Shell命令分析、综合安全检查、安全批准流程）',
  // 第二十一阶段新增 - Context Engine 插件化架构
  contextEngineRegistry: '上下文引擎注册表（插件注册、工厂管理、引擎解析、默认引擎设置、引擎ID列表）',
  contextEngineLegacy: '传统上下文引擎（消息摄入、上下文组装、智能压缩、Token估算、会话管理、100%向后兼容）',
  // 第二十二阶段新增 - Subagent 生命周期管理增强
  subagentLifecycle: '子代理生命周期管理器（完整生命周期追踪、完成消息流、自动清理、生命周期钩子、事件记录、状态统计）',
  // 第二十三阶段新增 - Session Runtime Model Override
  sessionRuntimeModel: '会话运行时模型覆盖（会话级别模型配置、动态模型切换、配置持久化、使用统计、自动清理过期配置）',
  // 第二十四阶段新增 - Skills Enhanced Module
  skillsEnhanced: '增强技能系统（技能版本管理、语义化版本比较、智能缓存管理、依赖图分析、拓扑排序、环境隔离、执行环境管理）',
  // 第二十五阶段新增 - Memory Vector Enhanced Module
  memoryVectorEnhanced: '增强向量记忆系统（多嵌入提供商支持、向量索引优化、混合搜索、时间衰减机制、向量压缩、持久化存储）'};
...
const TOOL_CACHE_TTL = 3600000; // 1小时缓存

// 获取或创建工具定义（内存优化）
function getOrCreateTool(moduleName, fnName, fn) {
  const cacheKey = `${moduleName}_${fnName}`;
  
  // 检查缓存
  if (toolCache.has(cacheKey)) {
    const cached = toolCache.get(cacheKey);
    if (Date.now() - cached.timestamp < TOOL_CACHE_TTL) {
      return cached.tool;
    }
  }
...
setInterval(() => {
  ...
  for (const [key, value] of toolCache.entries()) {
  ...
  if (cleaned > 0) {
    console.error(`[iFlow Node MCP] 清理了 ${cleaned} 个过期工具缓存，当前缓存大小: ${toolCache.size}`);
  }
}, TOOL_CACHE_TTL);
...
for (const modName of moduleList) {
  ...
  if (mod) {
    ...
    for (const [fnName, fn] of Object.entries(mod)) {
      if (typeof fn === 'function') {
        const tool = getOrCreateTool(modName, fnName, fn);
        tools.push(tool);
      }
    }
...
console.error(`[iFlow Node MCP] 工具缓存大小: ${toolCache.size}`);
